...
findLabel (const lineNode *pl)
{
  ...
  /* In each zr16 jumping opcode the label is at the end of the opcode */
         ...
         lineNode **plCond)
{
  ...
  for (rIdx = 0; rIdx < zr16_nRegs; ++rIdx)
    if (strcmp (regsZR16[rIdx].name, pReg + 1) == 0)
      break;
...
/*-----------------------------------------------------------------*/
/* zr16DeadMove - dispatch condition deadmove between             */
/* - remove pop/push                                               */
/* - remove mov r%1,%2                                             */
/*-----------------------------------------------------------------*/
bool
zr16DeadMove (const char *reg, lineNode *currPl, lineNode *head)
{
  char pReg[5] = "ar";
