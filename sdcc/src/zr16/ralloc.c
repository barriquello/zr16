...
/* Shared with gen.c */
int zr16_ptrRegReq;            /* one byte pointer register required */
...
reg_info regsZR16[] = {
  ...
  {REG_GPR, R9_IDX, REG_GPR, "R9", "r9", "0", 9, 1},	
  {REG_GPR, R8_IDX, REG_GPR, "R8", "r8", "0", 8, 1},
  {REG_GPR, R7_IDX, REG_GPR, "R7", "r7", "0", 7, 1},
  {REG_GPR, R6_IDX, REG_GPR, "R6", "r6", "0", 6, 1},
  {REG_GPR, R5_IDX, REG_GPR, "R5", "r5", "0", 5, 1},
  {REG_GPR, R4_IDX, REG_GPR, "R4", "r4", "0", 4, 1},
  {REG_GPR, R3_IDX, REG_GPR, "R3", "r3", "0", 3, 1},
  {REG_GPR, R2_IDX, REG_GPR, "R2", "r2", "0", 2, 1},
  {REG_GPR, R10_IDX, REG_GPR, "R10", "r10", "0", 10, 1},
  {REG_GPR, R11_IDX, REG_GPR, "R11", "r11", "0", 11, 1},
  {REG_GPR, R12_IDX, REG_GPR, "R12", "r12", "0", 12, 1},
  {REG_GPR, R13_IDX, REG_GPR, "R13", "r13", "0", 13, 1},
  {REG_GPR, R14_IDX, REG_GPR, "R14", "r14", "0", 14, 1},
  {REG_GPR, R1_IDX, REG_GPR, "R1", "r1", "0", 1, 1},

  {REG_CND, CND_IDX, REG_CND, "R15", "r15", "0x0F", 0, 1},
  {0, R0_IDX, 0, "R0", "r0", "0x00", 0, 1},
};
  ...
  for (i = 0; i < zr16_nRegs; i++)
    {
      ...
      /* otherwise look for specific type
         of register */
      if (regsZR16[i].isFree && regsZR16[i].type == type)
        {
          regsZR16[i].isFree = 0;
          if (currFunc)
            currFunc->regsUsed = bitVectSetBit (currFunc->regsUsed, i);
...
/*-----------------------------------------------------------------*/
/* zr16_regWithIdx - returns pointer to register with index number*/
/*-----------------------------------------------------------------*/
reg_info *
zr16_regWithIdx (int idx)
{
  int i;

  for (i = 0; i < sizeof (regsZR16) / sizeof (reg_info); i++)
    if (regsZR16[i].rIdx == idx)
      return &regsZR16[i];
...
spillLRWithPtrReg (symbol * forSym)
{
  ...
  r0 = zr16_regWithIdx (R0_IDX);
  r1 = zr16_regWithIdx (R1_IDX);
...
spillThis (symbol * sym)
{
  ...
  /* if spilt on stack then free up r0 & r1
     ...
     LIVE ranges */
  if (!zr16_ptrRegReq && isSpiltOnStack (sym))
    {
...
spilSomething (iCode * ic, eBBlock * ebp, symbol * forSym)
{
  ...
  if (!zr16_ptrRegReq && isSpiltOnStack (ssym))
    {
...
updateRegUsage (iCode * ic)
{
  ...
  for (reg = 0; reg < zr16_nRegs; reg++)
    {
      if (regsZR16[reg].isFree)
        {
          ic->riu &= ~(1 << regsZR16[reg].offset);
        }
      else
        {
          ic->riu |= (1 << regsZR16[reg].offset);
...
findAllBitregs (void)
{
  bitVect *rmask = newBitVect (zr16_nRegs);
  int j;

  for (j = 0; j < zr16_nRegs; j++)
    {
      if (regsZR16[j].type == REG_BIT)
        rmask = bitVectSetBit (rmask, regsZR16[j].rIdx);
    }
...
/*-----------------------------------------------------------------*/
/* zr16_allBitregs :- returns bit vector of all bit registers     */
/*-----------------------------------------------------------------*/
bitVect *
zr16_allBitregs (void)
{
...
findAllBankregs (void)
{
  ...
  for (j = 0; j < zr16_nRegs; j++)
    {
      if ((regsZR16[j].type == REG_GPR) || (regsZR16[j].type == REG_PTR))
        rmask = bitVectSetBit (rmask, regsZR16[j].rIdx);
    }
...
/*-----------------------------------------------------------------*/
/* zr16_allBankregs :- returns bit vector of all bank registers   */
/*-----------------------------------------------------------------*/
bitVect *
zr16_allBankregs (void)
{
...
regsUsedIniCode (iCode * ic)
{
  ...
  if (ic->op == IFX)
    {
      rmask = bitVectUnion (rmask, zr16_rUmaskForOp (IC_COND (ic)));
      goto ret;
    }
  ...
  if (ic->op == JUMPTABLE)
    {
      rmask = bitVectUnion (rmask, zr16_rUmaskForOp (IC_JTCOND (ic)));
      goto ret;
    }

  /* of all other cases */
  if (IC_LEFT (ic))
    rmask = bitVectUnion (rmask, zr16_rUmaskForOp (IC_LEFT (ic)));

  if (IC_RIGHT (ic))
    rmask = bitVectUnion (rmask, zr16_rUmaskForOp (IC_RIGHT (ic)));

  if (IC_RESULT (ic))
    rmask = bitVectUnion (rmask, zr16_rUmaskForOp (IC_RESULT (ic)));
      ...
      for (ic = ebbs[i]->sch; ic; ic = ic->next)
        {
          ...
          ic->rMask = newBitVect (zr16_nRegs + 1);
...
/*-----------------------------------------------------------------*/
bool
ZR16isCommutativeOp (unsigned int op)
{
...
/*-----------------------------------------------------------------*/
bool
ZR16operandUsesAcc (operand * op, bool allowBitspace)
{
  if (!op)
    return FALSE;
...
packRegsForAccUse (iCode * ic)
{
  ...
  if ((IC_LEFT (uic)->key == IC_RESULT (ic)->key &&
       ZR16operandUsesAcc (IC_RIGHT (uic), IS_BIT (operandType (IC_LEFT (uic))))) ||
      (IC_RIGHT (uic)->key == IC_RESULT (ic)->key && ZR16operandUsesAcc (IC_LEFT (uic), IS_BIT (operandType (IC_RIGHT (uic))))))
    return;
  ...
  if (!(ZR16isCommutativeOp (ic->op) || ic->op == '-') && IC_LEFT (uic)->key != IC_RESULT (ic)->key)
    return;
...
/*-----------------------------------------------------------------*/
void
zr16_assignRegisters (ebbIndex * ebbi)
{
  ...
  zr16_ptrRegReq = _G.stackExtend = _G.dataExtend = 0;
