/*-------------------------------------------------------------------------
  main.c - zr16 specific general functions
...
-------------------------------------------------------------------*/
/*
    Note that mlh prepended _zr16_ on the static functions.  Makes
...
/*
static OPTION _zr16_options[] = NULL;
  {
...
void zr16_assignRegisters (ebbIndex *);
...
_zr16_init (void)
{
  asm_addTree (&asm_zr16_mapping);//&asm_a390_mapping);//asm_asxxxx_mapping);
}

static void
_zr16_reset_regparm (void)
{
  regParmFlg = 0;
  regBitParmFlg = 0;
}
...
static int
_zr16_regparm (sym_link * l, bool reentrant)
{
...
static bool
_zr16_parseOptions (int *pargc, char **argv, int *i)
{
...
_zr16_finaliseOptions (void)
{
  ...
  /* zr16 has an assembly coded float library that's almost always reentrant */
...
static void
_zr16_setDefaultOptions (void)
{
}

static const char *
_zr16_getRegName (const struct reg_info *reg)
{
  if (reg)
    return reg->name;
  return "err";
...
static void
_zr16_genAssemblerPreamble (FILE * of)
{
	fprintf (of  , "warning_off\n\nDR sp = R10\n\n");
}

/* Generate interrupt vector table. */
static int
_zr16_genIVT (struct dbuf_s * oBuf, symbol ** interrupts, int maxInterrupts)
{
...
static void
_zr16_genExtraAreas(FILE *of, bool hasMain)
{
}

static void
_zr16_genInitStartup (FILE *of)
{
}


/* Generate code to copy XINIT to XISEG */
static void _zr16_genXINIT (FILE * of)
{
...
static bool cseCostEstimation (iCode *ic, iCode *pdic)
{
  ...
  /* if bitwise | add & subtract then no since zr16 is pretty good at it
...
typedef struct zr16operanddata
  {
    char name[6];
    int regIdx1;
    int regIdx2;
...
/*
static zr16operanddata zr16operandDataTable[] =
  {
    {"a",    A_IDX,   -1},
    {"ab",   A_IDX,   B_IDX},
    {"ac",   CND_IDX, -1},
...
static int
zr16operandCompare (const void *key, const void *member)
{
  return strcmp((const char *)key, ((zr16operanddata *)member)->name);
}
...
updateOpRW (asmLineNode *aln, char *op, char *optype)
{
  ...
  if (dot)
    *dot = '\0';
/*
  opdat = bsearch (op, zr16operandDataTable,
                   sizeof(zr16operandDataTable)/sizeof(zr16operanddata),
                   sizeof(zr16operanddata), zr16operandCompare);
*/
...
static zr16opcodedata zr16opcodeDataTable[] = {NULL};
/*
  {
    {"acall","j", "",   "",   ""},
    {"add",  "",  "w",  "rw", "r"},
...
static int
zr16opcodeCompare (const void *key, const void *member)
{
  return strcmp((const char *)key, ((zr16opcodedata *)member)->name);
}
...
asmLineNodeFromLineNode (lineNode *ln)
{
  ...
  *op = '\0';
  ...
  opdat = bsearch (inst, zr16opcodeDataTable,
                   sizeof(zr16opcodeDataTable)/sizeof(zr16opcodedata),
                   sizeof(zr16opcodedata), zr16opcodeCompare);
...
{
  "sdas8051", "$l", "$3", "$2", "$1.zr16", NULL
};

//static const char * const _libs[] = {"mcs51", STD_LIB, STD_INT_LIB, STD_LONG_LIB, STD_FP_LIB, NULL, };
static const char * const _libs[] = {"zr16", STD_LIB, STD_INT_LIB, STD_LONG_LIB, STD_FP_LIB, NULL, };
...
PORT zr16_port =
{
  ...
  {
    /* zr16 has an 8 bit mul */
    1, -1
  },
  {
   zr16_emitDebuggerSymbol
  },
  {
    256,        /* maxCount */
    2,          /* sizeofElement */
  ...
  NULL,//_zr16_options,
  NULL,
  _zr16_finaliseOptions,
  _zr16_setDefaultOptions,
  zr16_assignRegisters,
  _zr16_getRegName,
  _zr16_rtrackUpdate,
  _zr16_keywords,
  _zr16_genAssemblerPreamble,
  NULL,                         /* no genAssemblerEnd */
  _zr16_genIVT,
  _zr16_genXINIT,
  _zr16_genInitStartup,
  _zr16_reset_regparm,
  _zr16_regparm,
  NULL,                         /* process_pragma */
