...
static void emitRegularMap(memmap * map, bool addPublics, bool arFlag)
{
	...
	for (sym = setFirstItem(map->syms); sym; sym = setNextItem(map->syms))
	{
		...
		if (arFlag && !IS_AGGREGATE(sym->type) && !(sym->_isparm && !IS_REGPARM(sym->etype)) && !sym->allocreq
				&& sym->level)
			continue;
		...
		if ((sym->level == 0 || (sym->_isparm && !IS_REGPARM(sym->etype) && !IS_STATIC(sym->localof->etype)))
				&& addPublics && !IS_STATIC(sym->etype)
				&& (IS_FUNC(sym->type) ?
						(sym->used || IFFUNC_HASBODY(sym->type)) : (!IS_EXTERN(sym->etype) || sym->ival))
		...
		else
		{
			...
			{   
				if (TARGET_IS_ZR16 && !strstr (sym -> rname , "IO"))
				{
					if (getSize(sym->type) <= 2)
					{
						dbuf_tprintf(&map->oBuf, "DA %d {%s_L}\n", varAddress , sym->name);
						dbuf_tprintf(&map->oBuf, "DV ptr_%s_L = %d\n" , sym->name , varAddress++);
					}
					// Creates DV for 2 B var.
					if (getSize(sym->type) == 2 && !IS_ARRAY(sym -> type))
					{
						dbuf_tprintf(&map->oBuf, "DA %d {%s_H}\n", varAddress , sym->name);
						dbuf_tprintf(&map->oBuf, "DV ptr_%s_H = %d\n", sym->name , varAddress++); // Used to assign pointers.
					}
					if (IS_PTR (sym -> type))
					{
						dbuf_tprintf(&map->oBuf, "DA %d {%s_L}\n", varAddress++ , sym->name);
					...
					else if (IS_ARRAY(sym -> type))  // vectors
					{
						int i;
	
						dbuf_tprintf(&map->oBuf, "\nDV %s = %d\n" , sym->name , varAddress);
	
						for (i = 0 ; i < size ; i++)
						{
							dbuf_tprintf(&map->oBuf, "DA %d {%s%d}\n" , varAddress++, sym->name , i);
						}
					}
				}

				else if (!strstr (sym -> rname , "IO"))
...
initValPointer(ast * expr)
{
	...
	if (IS_AST_OP(expr) && (expr->opval.op == '+' || expr->opval.op == '-') && IS_CAST_OP(expr->left)
			&& IS_ADDRESS_OF_OP(expr->left->right) && IS_AST_LIT_VALUE(expr->right))
	{
...
initPointer(initList * ilist, sym_link * toType)
{
	...
	/* ( ptr + constant ) */
	if (IS_AST_OP(expr) && (expr->opval.op == '+' || expr->opval.op == '-') && IS_AST_SYM_VALUE(expr->left)
			&& (IS_ARRAY(expr->left->ftype) || IS_PTR(expr->left->ftype)) && compareType(toType, expr->left->ftype)
...
/*-----------------------------------------------------------------*/
static void printGPointerType(struct dbuf_s *oBuf, const char *iname, const char *oname, int type)
{
	...
	switch (getSize(type))
	{
		...
		case 4:
			...
			else
			{
				if (port->little_endian)
				{
					dbuf_printf(oBuf, "\t.byte %s,%s,%s,%s", aopLiteral(val, 0), aopLiteral(val, 1), aopLiteral(val, 2),
							aopLiteral(val, 3));
				}
				else
				{
					dbuf_printf(oBuf, "\t.byte %s,%s,%s,%s", aopLiteral(val, 3), aopLiteral(val, 2), aopLiteral(val, 1),
							aopLiteral(val, 0));
				}
		...
		case 8:
			if (port->little_endian)
			{
				dbuf_printf(oBuf, "\t.byte %s,%s,%s,%s,%s,%s,%s,%s", aopLiteral(val, 0), aopLiteral(val, 1),
						aopLiteral(val, 2), aopLiteral(val, 3), aopLiteral(val, 4), aopLiteral(val, 5),
						aopLiteral(val, 6), aopLiteral(val, 7));
			}
			else
			{
				dbuf_printf(oBuf, "\t.byte %s,%s,%s,%s,%s,%s,%s,%s", aopLiteral(val, 7), aopLiteral(val, 6),
						aopLiteral(val, 5), aopLiteral(val, 4), aopLiteral(val, 3), aopLiteral(val, 2),
...
/*-----------------------------------------------------------------*/
static void printIvalBitFields(symbol ** sym, initList ** ilist, struct dbuf_s *oBuf)
{
	symbol *lsym = *sym;
	...
	switch (size)
	{
		...
		case 4:
			dbuf_tprintf(oBuf, "\t!db !constbyte, !constbyte, !constbyte, !constbyte\n", (ival & 0xff),
					(ival >> 8) & 0xff, (ival >> 16) & 0xff, (ival >> 24) & 0xff);
			break;
	}
	*sym = lsym;
	*ilist = lilist;
...
/*-----------------------------------------------------------------*/
static void printIvalStruct(symbol * sym, sym_link * type, initList * ilist, struct dbuf_s *oBuf)
{
	symbol *sflds;
	initList *iloop = NULL;
...
void printIvalArray(symbol * sym, sym_link * type, initList * ilist, struct dbuf_s *oBuf, bool check)
{
	...
	if (ilist)
	{
		...
		if (IS_CHAR(type->next) && ilist->type == INIT_NODE)
		{
			...
			if (printIvalChar(sym, type, (ilist->type == INIT_DEEP ? ilist->init.deep : ilist), oBuf,
		...
		switch (size)
		{
			...
			case 4:
				if (IS_GENPTR(type) && GPTRSIZE > FPTRSIZE && floatFromVal(val) != 0)
				{
					...
					if (port->little_endian)
					{
						dbuf_printf(oBuf, "\t.byte %s,%s,%s", aopLiteral(val, 0), aopLiteral(val, 1),
								aopLiteral(val, 2));
	...
	for (sym = setFirstItem(map->syms); sym; sym = setNextItem(map->syms))
	{
		...
		else
		{
			...
			if (sym->ival)
			{
				...
				if (IS_ARRAY(sym->type) && IS_CHAR(sym->type->next) && IS_AST_SYM_VALUE(list2expr(sym->ival))
	...
	for (ovrset = setFirstItem(ovrSetSets); ovrset; ovrset = setNextItem(ovrSetSets))
	{
		...
		for (sym = setFirstItem(ovrset); sym; sym = setNextItem(ovrset))
		{
			...
			else
			{
				...
				if (TARGET_IS_ZR16)
				{
					strcpy (param_buf[contParams] , sym -> rname);
					int index = param_buf[contParams][strlen(param_buf[contParams])-1] - 48;
					if (index > maxNrOfParameters)
					{
						maxNrOfParameters = index - 1;
					}
					contParams++;
				}
		...
		if (TARGET_IS_ZR16)
		{
			...
			for (i = 0 ; i < contParams ; i++)
			{
				strcpy (sel , param_buf[i]);
				index1 = sel[strlen(sel)-1];
		
				if (indexIsInBucket (index1))
				{ 
					continue;
				}
				...
				dbuf_tprintf(aBuf,"DA %d {%s_L", varAddress , sel);
				dbuf_tprintf(aBuf,"}\n");

				for (j = 0 ; j < contParams ; j++)
				{
					index2 = param_buf[j][strlen(param_buf[j])-1];

					if (index1 == index2 && strcmp (sel , param_buf[j]))
					{
						dbuf_tprintf(aBuf,"DA %d {%s_L}\n" ,varAddress , param_buf[j]);
					}
				}	


				dbuf_tprintf(aBuf,"DA %d {%s_H", ++varAddress , sel);
				dbuf_tprintf(aBuf,"}\n");
			...
			// DA 1 {x_L}
			// DA 2 {x_H}
			//dbuf_tprintf(aBuf, "DA %d {dpl}\n", varAddress++);
			//dbuf_tprintf(aBuf, "DA %d {dph}\n", varAddress++);
		}
}
...
void glue(void)
{
	...
	/* print module name */
	if (!TARGET_IS_ZR16) tfprintf(asmFile, "\t!module\n", moduleName);
	if (mcs51_like)
	{
	...
	else if (!TARGET_PIC_LIKE && !options.noOptsdccInAsm && !TARGET_IS_ZR16)
	{
		fprintf(asmFile, "\t.optsdcc -m%s\n", port->target);
	}
	
	if (!TARGET_IS_ZR16) tfprintf(asmFile, "\t!fileprelude\n");
	...
	if (mainf && IFFUNC_HASBODY(mainf->type))
	{
		...
		/* put in jump or call to main */
			if (TARGET_IS_STM8 && !TARGET_IS_ZR16)
